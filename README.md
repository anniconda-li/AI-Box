...
后端会判断这类句子像追问，并把 `latest_artifact_id` 对应的知识卡再次加入 LLM 上下文。追问判断不是只靠完整句子匹配，也会识别“这/它/刚才/上一件”等指代词和“什么/故事/用途/年代/为什么”等问题词的组合。这样回答不是单纯依赖上一轮 assistant 的文本记忆，而是明确继承“当前设备正在看的那件文物”。
